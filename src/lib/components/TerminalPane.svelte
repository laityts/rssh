<script lang="ts">
    ...
    onMount(async () => {
        ...
        if (app.isMobile) {
            ...
            // 块复制 / 发 AI 取文，这个取舍可接受。
            // Alt-buffer drags become cursor keys (see touch-scroll.ts). Raw
            // control-sequence path — same as writePty, NOT sendText (no newline
            // discipline). Serial has no cursor-key scrolling semantics, so skip.
            const ptyKeySink = (seq: string) => {
                if (!sessionId || disconnected || tabType === "serial") return;
                invoke(writeCmd, { sessionId, data: Array.from(new TextEncoder().encode(seq)) });
            };
            mobileTouchScrollCleanup = setupTouchScroll(containerEl, terminal, ptyKeySink);
        }
